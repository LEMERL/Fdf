...
int		ft_col(t_trace t, t_env *e)
{
	...
	if (vcol.degradee > 0)
		ft_degradee(t.c1, t.c2, p, t.ref_prog);
	else
...
t_color	ft_degradee(t_color c1, t_color c2, int p, int ref_prog)
{
	...
	prog = ((p * 255) / ref_prog) / 4;
	prog = prog * 4;
	if ((((p * 255) / ref_prog) % 4) >= (2))
		prog = prog + 4;
	if (prog > 255)
		prog = 255;
	...
	if (vcol.col_flag >= 2)
	{
		if (strc->z < 0)
			strc->aff_c = ft_degradee(vcol.col_underground, vcol.col_ground,
					strc->z, cst.low_z);
		if (strc->z > 0)
			strc->aff_c = ft_degradee(vcol.col_ground, vcol.col_ground,
					strc->z, cst.max_z);
		if (strc->z == 0)
			strc->aff_c = vcol.col_ground;
	}
